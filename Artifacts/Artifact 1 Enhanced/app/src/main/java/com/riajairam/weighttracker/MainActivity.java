...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textResendVerification.setOnClickListener(v -> {
            ...
            if (user != null) {
                ...
                // fallback (usually won't happen now)
    ...
    protected void onStart() {
        ...
        if (user != null) {
            if (!user.isEmailVerified()) {
                // IMPORTANT: do NOT sign out here, otherwise resend may fail
                showResendLink(user);
                return;
            }
    ...
    private void createAccount() {
        ...
        auth.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(task -> {
                    ...
                    FirebaseUser user = auth.getCurrentUser();
                    if (user != null) {
                        // Send verification; resendVerificationEmail() will sign out afterward
                        resendVerificationEmail(user);
                        showResendLink(user);
                    }
                });
    ...
    private void showResendLink(FirebaseUser user) {
        ...
        Toast.makeText(this,
                "Email not verified. Tap to resend verification (check spam).",
                Toast.LENGTH_LONG).show();
    }
    ...
    private void resendVerificationEmail(FirebaseUser user) {
        user.sendEmailVerification()
                .addOnCompleteListener(task -> {
                    ...
                    // Now sign out to force verify-before-use
                    auth.signOut();
                    lastUnverifiedUser = null;
    ...
    private void routeAfterAuth(FirebaseUser user) {
        ...
        boolean hasGoal = userHasGoal(email);
    ...
    private boolean userHasGoal(String email) {
        List<String[]> goalData = db.getGoalWeight(email);
        return goalData != null && !goalData.isEmpty();
    }
